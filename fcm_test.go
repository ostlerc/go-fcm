...
func TestHttpClientSend(t *testing.T) {
	...
	c := &httpFcmClient{server.URL, httpClient, "0"}
...
// test sending a FCM message through the HTTP connection server (includes backoff)
...
func TestXmppUser(t *testing.T) {
	assertEqual(t, xmppUser("b"), "b@fcm-xmpp.googleapis.com")
}
