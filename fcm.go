...
var (
	...
	xmppClients = struct {
		sync.Mutex
		m map[string]*xmppFcmClient
	}{
		m: make(map[string]*xmppFcmClient),
	}
)
...
// httpFcmClient implementation to send a message through FCM Http server.
func (c *httpFcmClient) send(apiKey string, m HttpMessage) (*HttpResponse, error) {
	...
	if err != nil {
		return fcmResp, fmt.Errorf("error reading http response body>%v", err)
	}
...
func (c httpFcmClient) getRetryAfter() string {
	return c.retryAfter
}
...
// xmppFcmClient is a client for the Fcm Xmpp Connection Server (CCS).
type xmppFcmClient struct {
	sync.RWMutex
...
// Factory method for xmppFcmClient, to minimize the number of clients to one per sender id.
// TODO(silvano): this could be revised, taking into account that we cannot have more than 1000
// connections per senderId.
func newXmppFcmClient(senderID string, apiKey string) (*xmppFcmClient, error) {
	xmppClients.Lock()
...
func sendHttp(apiKey string, m HttpMessage, c httpClient, b backoffProvider) (*HttpResponse, error) {
	...
	if err != nil {
		return fcmResp, fmt.Errorf("error extracting target from message: %v", err)
	}
...
func checkResults(fcmResults []Result, recipients []string, resultsState multicastResultsState) (doRetry bool, toRetry []string, err error) {
	doRetry = false
	toRetry = []string{}
...
func SendXmpp(senderId, apiKey string, m XmppMessage) (string, int, error) {
	c, err := newXmppFcmClient(senderId, apiKey)
	if err != nil {
